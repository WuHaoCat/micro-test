...
int main(int argc,char *argv[]){
    ...
    int arraySize = 513*1024;
    int stride = 1;
    unsigned int iter = arraySize/8 + 100;
    int cacheMissRate = 0;
    ...
    choosePlatform();
    chooseDevice();
    cl_program program = buildProgram("cachesize.cl");
	...
	ckEr(status, __LINE__);
	cl_kernel kernel;
	kernel=clCreateKernel(program,"kernel_cacheSize",&status);
	ckEr(status, __LINE__);
	//Data Set Up
	...
	ckEr(status, __LINE__); 
	status = clSetKernelArg(kernel, 2, sizeof(cl_uint), &iter);
    ckEr(status, __LINE__); 
    status = clSetKernelArg(kernel, 3, sizeof(cl_uint), &arraySize);
	ckEr(status, __LINE__);
