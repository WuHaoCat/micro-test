...
int main(int argc,char *argv[]){
    ...
    float* re_arr = new float[arraySize];
    float* re_arr_2 = new float[arraySize];
    ...
    chooseDevice();
	char clFileName[50];
	sprintf(clFileName, "regTest%d.cl", progTest);
    cl_program program = buildProgram(clFileName);
    sprintf(clFileName, "regTest%d.cl", (52-progTest));
    cl_program program_2 = buildProgram(clFileName);
    cl_int status;
    ...
    ckEr(status, __LINE__);
    cl_kernel kernel_2;
    kernel_2 = clCreateKernel(program_2,"regTest",&status);
    ckEr(status, __LINE__);
    ...
    cl_mem input_2 = clCreateBuffer(context,  CL_MEM_READ_ONLY, arraySize*sizeof(cl_int), NULL, &status);
    ckEr(status, __LINE__);
    cl_mem output_2 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, arraySize*sizeof(cl_uint), NULL, &status);
    ckEr(status, __LINE__);
    //Data Transfer
    status = clEnqueueWriteBuffer(cmdQueue, input_2, CL_TRUE, 0, arraySize*sizeof(cl_int), arr, 0, NULL, NULL);
    ckEr(status, __LINE__);

	size_t global[1] = {arraySize};
    ...
    status = clFinish(cmdQueue);
    ckEr(status, __LINE__);
    status = clEnqueueReadBuffer(cmdQueue_2, output_2, CL_TRUE, 0, arraySize*sizeof(cl_uint), re_arr_2, 0, NULL, NULL);
    ckEr(status, __LINE__);
    status = clFinish(cmdQueue);
    ckEr(status, __LINE__);
    ...
    f<<parallel<<","<<progTest<<","<<start1<<","<<end1<<","<<start2<<","<<end2<<endl;
    f.close();
